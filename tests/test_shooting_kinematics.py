...
def test_values_between_lookup_points_are_interpolated():
    ...
    assert kinematics.calculateShootingAngle(1.25) == 11.25
    assert kinematics.calculateTimeOfFlight(1.25) == 0.175
    assert kinematics.calculateShootingSpeed(6.25) == 5975.0
    assert kinematics.calculateShootingAngle(6.25) == 16.25
...
def test_calculate_shot_returns_speed_angle_and_time_of_flight():
    ...
    assert kinematics.calculateShot(2.5) == (5400.0, 12.5, 0.25)


def test_custom_lookup_table_is_sorted_before_interpolation():
    kinematics = ShootingKinematics(
        lookupTable=[
            (10.0, 6500.0, 40.0, 0.8),
            (0.0, 5000.0, 10.0, 0.1),
            (5.0, 6000.0, 20.0, 0.5),
        ]
    )

    assert kinematics.calculateShot(2.5) == pytest.approx((5500.0, 15.0, 0.3))


def test_lookup_table_requires_at_least_two_points():
    with pytest.raises(ValueError, match="at least two points"):
        ShootingKinematics(lookupTable=[(0.0, 5000.0, 10.0, 0.1)])
...
def test_lookup_table_rejects_angle_outside_limits():
